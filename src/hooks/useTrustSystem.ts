...
export function useTrustSystem() {
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      const mockTasks = [
        { id: '1', title: 'Smart Contract Audit', trustScore: 80, creator: 'Anonymous', completed: false },
        { id: '2', title: 'Frontend Design', trustScore: 40, creator: 'Alice', completed: true },
      ];
  ...
  const createTask = async (input: CreateTaskInput, user: string) => {
    ...
    setTasks(prev => [...prev, { id: Date.now().toString(), title: input.title, description: input.description, trustScore: 0, creator: user, completed: false } as any]);
  };

  const endorseTask = async (input: { taskId: string; amount: number }, user: string, signer?: any) => {
